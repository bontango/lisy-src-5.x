...
main(int argc, char** argv) {
    ...
    if (argc < 4) {
        printf("Usage: %s <Switch> <1|0> <time in ms> .. sets <Switch> on or off plus waittime ahaed, uses UDB via "
               "localhost \n",
               argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    rc = sendto(s, &data, 1, 0, (struct sockaddr*)&remoteServAddr, sizeof(remoteServAddr));
    if (rc < 0) {
