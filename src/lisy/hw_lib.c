...
void
lisy80_hwlib_wiringPI_init(void) {
    // wiringPi library
    if (wiringPiSetup() < 0)
        lisy80_error(1);
}
...
// -1 in case of error
int
lisy_get_hardware_revision(int disp_sw_ver, int* hw_ID) {
...
lisymini_hwlib_init(void) {
    ...
    if (ls80dbg.bitv.basic)
        lisy80_debug("LISY_Mini Hardware init start");
...
lisyapc_hwlib_init(void) {
    ...
    //lets do debug
    if (ls80dbg.bitv.basic)
        lisy80_debug("LISY APC Hardware init start");
...
lisy_hwlib_init(void) {
    ...
    if (ls80dbg.bitv.basic)
        lisy80_debug("LISY Hardware init start");

    //******************
    // PIC 1 - Displays
    //******************
    if (ls80dbg.bitv.basic)
        lisy80_debug("display PIC init start");
    ...
    //******************
    if (ls80dbg.bitv.basic)
        lisy80_debug("coil PIC init start");
    ...
    pinMode(LISY80_SWPIC_RESET, OUTPUT);
    ...
    if (lisy_hardware_revision == 311)
        ...
        pinMode(LISY80_HW320_LED_RED, OUTPUT);
...
int
lisy80_write_multibyte_pic(int fd, char* buf, int no) {
    int bytes_written;
...
int
lisy80_write_byte_pic(int fd, unsigned char buf) {
    int no;
    int repeats = 0;
    char device[20];
    ...
    while (++repeats <= 5) {
        if ((no = write(fd, &buf, 1)) == 1) //success
            ...
            if (fd == fd_disp_pic)
                strcpy(device, "Display Pic");
            else
                strcpy(device, "Coil Pic");
...
//------ DISPLAYS ------
int
lisy80_write_byte_disp_pic(unsigned char buf) {
    return (lisy80_write_byte_pic(fd_disp_pic, buf));
}

int
lisy80_write_multibyte_disp_pic(char* buf, int no) {
    return (lisy80_write_multibyte_pic(fd_disp_pic, buf, no));
}

//------ COILS ------
int
lisy80_write_byte_coil_pic(unsigned char buf) {
    return (lisy80_write_byte_pic(fd_coil_pic, buf));
}

int
lisy80_write_multibyte_coil_pic(char* buf, int no) {
...
unsigned char
lisy80_read_byte_disp_pic(void) {
    return (lisy80_read_byte_pic(fd_disp_pic));
}

//read one byte from coil PIC
unsigned char
lisy80_read_byte_coil_pic(void) {
    return (lisy80_read_byte_pic(fd_coil_pic));
}

//set the sound LISY1
void
lisy1_sound_set(int sound) {

    int mysound;

    //sound is solenoids 3..7, so need to shift here
    if (sound & 0x01)
        lisy1_coil_set(Q_TENS, 1);
    else
        lisy1_coil_set(Q_TENS, 0);
    if (sound & 0x02)
        lisy1_coil_set(Q_HUND, 1);
    else
        lisy1_coil_set(Q_HUND, 0);
    if (sound & 0x04)
        lisy1_coil_set(Q_TOUS, 1);
    else
...
lisy80_switch_pic_init(void) {
    ...
    if ((subversion > 2) || (version > 3)) {
        ...
        K1_debug_values = 0; //no debug for subversion <3!
...
int
lisy80_switch_reader(unsigned char* action) {
...
int
lisy35_switch_reader(unsigned char* action) {
...
int
lisy1_switch_reader(unsigned char* action) {
...
int
lisy80_dip1_debug_option(void) {

    int setting;

    //we support different hardware revisions
    if (lisy_hardware_revision == 311) {
        pinMode(LISY80_HW311_DIP1_S7, INPUT);
        pullUpDnControl(LISY80_HW311_DIP1_S7, PUD_UP);
        setting = digitalRead(LISY80_HW311_DIP1_S7);
    } else {
        pinMode(LISY80_HW320_DIP1_S7, INPUT);
        pullUpDnControl(LISY80_HW320_DIP1_S7, PUD_UP);
...
void
lisy80_get_dips(void) {

    //we support different hardware revisions
    if (lisy_hardware_revision == 311) {
        //set the mode for DIP Switch 1, Input with pull up enable, 'ON' means pulled to GND
        pinMode(LISY80_HW311_DIP1_S1, INPUT);
        pullUpDnControl(LISY80_HW311_DIP1_S1, PUD_UP);
        pinMode(LISY80_HW311_DIP1_S2, INPUT);
        pullUpDnControl(LISY80_HW311_DIP1_S2, PUD_UP);
        pinMode(LISY80_HW311_DIP1_S3, INPUT);
        pullUpDnControl(LISY80_HW311_DIP1_S3, PUD_UP);
        pinMode(LISY80_HW311_DIP1_S4, INPUT);
        pullUpDnControl(LISY80_HW311_DIP1_S4, PUD_UP);
        pinMode(LISY80_HW311_DIP1_S5, INPUT);
        pullUpDnControl(LISY80_HW311_DIP1_S5, PUD_UP);
        pinMode(LISY80_HW311_DIP1_S6, INPUT);
        pullUpDnControl(LISY80_HW311_DIP1_S6, PUD_UP);
        pinMode(LISY80_HW311_DIP1_S7, INPUT);
        pullUpDnControl(LISY80_HW311_DIP1_S7, PUD_UP);
        pinMode(LISY80_HW311_DIP1_S8, INPUT);
        pullUpDnControl(LISY80_HW311_DIP1_S8, PUD_UP);
    ...
    } else if (lisy_hardware_revision == LISY_HW_LISY_APC) //lisy integrated on APC
    {
        ...
        if (CHECK_BIT(K1_debug_values, 0))
            ls80dbg.bitv.displays = 1;
        else
            ls80dbg.bitv.displays = 0;
        if (CHECK_BIT(K1_debug_values, 1))
            ls80dbg.bitv.switches = 1;
        else
            ls80dbg.bitv.switches = 0;
        if (CHECK_BIT(K1_debug_values, 2))
            ls80dbg.bitv.lamps = 1;
        else
            ls80dbg.bitv.lamps = 0;
        if (CHECK_BIT(K1_debug_values, 3))
            ls80dbg.bitv.coils = 1;
        else
            ls80dbg.bitv.coils = 0;
        if (CHECK_BIT(K1_debug_values, 4))
            ls80dbg.bitv.sound = 1;
        else
            ls80dbg.bitv.sound = 0;
    } else {
        //set the mode for DIP Switch 1, Input with pull up enable, 'ON' means pulled to GND
        //we have only 4 from GPIO as the others are coming from switch PIC
        pinMode(LISY80_HW320_DIP1_S2, INPUT);
        pullUpDnControl(LISY80_HW320_DIP1_S2, PUD_UP);
        pinMode(LISY80_HW320_DIP1_S6, INPUT);
        pullUpDnControl(LISY80_HW320_DIP1_S6, PUD_UP);
        pinMode(LISY80_HW320_DIP1_S7, INPUT);
        pullUpDnControl(LISY80_HW320_DIP1_S7, PUD_UP);
        pinMode(LISY80_HW320_DIP1_S8, INPUT);
        pullUpDnControl(LISY80_HW320_DIP1_S8, PUD_UP);
    ...
    if (ls80dbg.bitv.basic == 1) {
        ...
        if (ls80opt.bitv.slam)
            lisy80_debug("LISY option: slam");
        if (ls80opt.bitv.test)
            lisy80_debug("LISY option: test");
        if (ls80opt.bitv.debug)
            lisy80_debug("LISY option: debug");
        if (ls80opt.bitv.autostart)
            lisy80_debug("LISY option: autostart");
    }
    ...
    if (ls80dbg.bitv.basic == 1) {
        if (ls80dbg.bitv.displays)
            lisy80_debug("LISY DEBUG activ for displays");
        if (ls80dbg.bitv.switches)
            lisy80_debug("LISY DEBUG activ for switches");
        if (ls80dbg.bitv.lamps)
            lisy80_debug("LISY DEBUG activ for lamps");
        if (ls80dbg.bitv.coils)
            lisy80_debug("LISY DEBUG activ for coils");
        if (ls80dbg.bitv.sound)
            lisy80_debug("LISY DEBUG activ for sound");
    } else
        ls80dbg.byte = 0; //override settings from stiftleiste, in case S2 is set to OFF, no debug
...
void
lisy35_switch_pic_init(unsigned char variant) {
    ...
    //   2        1      1
    if (ls80dbg.bitv.basic) {
        sprintf(debugbuf, "Info: LISY35 will use switch variant:%d ( next 4 switch_reader actions to be ignored)",
                variant);
        lisy80_debug(debugbuf);
    }
    if (variant == 1) {
