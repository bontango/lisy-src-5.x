            ...
            if (ret >= 0) //no error, interpret command
            {
                switch (cmd) {
                    ...
                    case LISY1_ATTRACT_CMD_LAMP:
                        if (opt == LISY1_ATTRACT_CMD_LAMP_ON) {
                            lisy1_coil_set(num, 1);
...
int
lisy1_get_gamename(char* gamename) {
...
//display handler
void
lisy1_display_handler(int index, int value) {

    static int myvalue[48] = {80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80,
...
*/
        ...
        switch (
            index) //we need to recalculate index each time as it has the reverse order for LISY1 compared with LISY80
        {
...
*/
unsigned char
lisy1_switch_handler(int sys1strobe) {
    int ret;
    ...
    if (ret < 80) //ret is switchnumber
    {
        ...
        if (ls80opt.bitv.ballsave == 1) {
            ...
            if (lisy_ball_save_event_handler(LISY_BALL_SAVE_EVENT_SWITCH, ret, action) == 1)
                return (swMatrixLISY1[sys1strobe - 1]);
        }
...
void
lisy1_solenoid_handler(int ioport, int enable) {
    ...
    switch (ioport) {
        case 0:
            lisy1_coil_set(Q_OUTH, !enable);
            break;
        case 1:
            lisy1_coil_set(Q_KNOCK, !enable);
            break;
        case 2:
        ...
        case 5:
            ...
            break;
        case 6:
            lisy1_coil_set(Q_SYS1_SOL7, !enable);
            break;
        case 7:
            lisy1_coil_set(Q_SYS1_SOL8, !enable);
            break;
    }
...
void
lisy1_lamp_handler(int data, int isld) {
    ...
    } else //we have a DS value now
    {
        ...
        if ((flip_flop_no < 1) || (flip_flop_no > 9))
            return;
        ...
        for (i = 0; i <= 3; i++) {
            if (lisy1_lamp[i + offset] != new_lamp[i]) {
                if (ls80dbg.bitv.lamps) {
                    if (new_lamp[i])
                        sprintf(debugbuf, "LISY1_LAMP_HANDLER: Transistor_Q:%d ON", i + 1 + offset);
                    else
                        sprintf(debugbuf, "LISY1_LAMP_HANDLER: Transistor_Q:%d OFF", i + 1 + offset);
                    lisy80_debug(debugbuf);
                }
                ...
                if (new_lamp[i])
                    lisy1_coil_set(i + 1 + offset, 1);
                else
                    lisy1_coil_set(i + 1 + offset, 0);
                ...
                switch (i + 1 + offset) //This is the transistor number
                {
                    case 1:
                        ...
                        if (ls80opt.bitv.ballsave)
                            lisy_ball_save_event_handler(LISY_BALL_SAVE_EVENT_GAME_OVER, new_lamp[0], 0);
                        break;
                    case 2:
                        if (Q2_first_time) {
                            Q2_first_time = 0;
...
lisy1TickleWatchdog(void) {
    ...
    if (lisy1_nvram_delayed_write) {
        ...
        if (lisy_timer(500, 0, 3)) {
            ...
            //debug
            if (ls80dbg.bitv.basic)
                lisy80_debug("NVRAM delayed write done ");
        }
...
//switch_nr is 0..2
int
lisy1_get_mpudips(int switch_nr) {
    ...
    //debug?
    if (ls80dbg.bitv.basic) {
        sprintf(debugbuf, "LISY1: switch_no:%d ret:%d DIP switch settings according to %s", switch_nr, ret,
                dip_file_name);
        lisy80_debug(debugbuf);
    }
...
unsigned char
lisy1_nvram_handler_old(int read_or_write, unsigned char ramAddr, unsigned char accu) {
