...
struct stru_lisy80_error lisy80_error_struct[13] = {
    ...
    {6, 0, 0, 0, "i2cbus", "I2C BUS PROB WRIT",
     "Failed to write to the I2C bus display pic"}, //RTH: no exit, need to be monitored for repeat
    {7, 0, 0, 0, "i2cbus", "I2C BUS PROB WRIT",
     "Failed to write to the I2C bus coil pic"}, //RTH: no exit, need to be monitored for repeat
    ...
    {10, 0, 0, 1, "romiss", "ROM MISSING      ",
     "Failed to read ROM data for selected game"}, //mame will do exit for us
    {11, 1, 0, 1, "inv hw", "INVALID HARDWARE ",
     "Could not determine Hardware revision"}, //unable to detect eeprom at valid address
...
void
lisy80_error(int error_num) {

    //put Error LED on
    lisy80_set_red_led(1);
    //put display LED to on if requested
    if (lisy80_error_struct[error_num].LED)
        lisy80_set_dp_led(1);

    //try to give error message at Gottlieb display if requested
    if (lisy80_error_struct[error_num].display)
        display_show_error_message(lisy80_error_struct[error_num].lisy80_err_number,
                                   lisy80_error_struct[error_num].message_short,
                                   lisy80_error_struct[error_num].message_long);
...
void
syserr(char* msg, int number, int doexit) {
...
lisy_init(void) {
    ...
    if (lisy_hardware_revision == 100)
        ...
        system = 121; // lise W <-> mini
    else if (lisy_hardware_revision == LISY_HW_LISY_APC)
        system = 122; // integrated with APC
    else if (lisy_hardware_revision == LISY_HW_LISY_H_SS)
        system = 200; // LISY_Home Starship
    else
        system = 80;
    ...
    if (lisy_hardware_revision == 100)
        ...
        lisy35_init();
    else if (lisy_hardware_revision == 121)
        lisy_w_init();
    else if (lisy_hardware_revision == 122)
        lisy_w_init();
    else if (lisy_hardware_revision == 200)
        lisy35_ss_init(); //LISY_Home Starship
    else
        lisy80_init();
}
...
//
unsigned char
LISY_SW_BufferIn(int index, unsigned char byte) {
...
//
unsigned char
LISY_SW_BufferOut(int index, unsigned char* pByte) {
...
int
lisy_timer(unsigned int duration, int command, int index) {
    unsigned int now;
    int millis_to_go;
    ...
    if (command == 0) {
        ...
        } else //give back status of timer
        {
            ...
            if (now < timer_last_on[index])
                now = timer_last_on[index]; //we had a wrap
...
void
my_itoa(int value, char* str, int base) {
...
int
lisy_udp_switch_reader(unsigned char* action, unsigned char do_only_init) {
    ...
    if (!do_only_init) //this is with do_only_init==0, and needs to be polled by normal switchreader
    {
        ...
        if (n < 0) {
            if (errno == EWOULDBLOCK)
                return (80); //return 80 indicates no data
...
void
lisy80_debug_swreplay(int sw, int action) {
    struct timeval now;
    static struct timeval last;
    long seconds, useconds;
    static int first = 1;
