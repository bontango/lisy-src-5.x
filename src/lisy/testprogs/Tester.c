...
send_to_80b(char* str) {
    ...
    switch (row) {
        case 1:
            display_sendtorow(1, 0, data, 0);
            break;
        case 2:
            display_sendtorow(0, 1, data, 0);
            break;
        case 3:
            display_sendtorow(1, 1, data, 0);
            break;
    }
}
...
lisy_help(int position) {
    ...
    switch (pos) {
        ...
        case 1:
            ...
            printf(" w show hw revision stored at display pic ( PIC Version 4.x only)\n");
        ...
        case 2:
            ...
            printf(" k show value of K3\n");
            printf(" m show value of MPU dips (LISY35 only)\n");
            printf(" Up\n");
            printf(" Help\n");
            break;
        //SWITCH
...
void
read_next_disppic_byte(void) {
    printf("Display PIC next byte in buffer:%d\n\r", lisy80_read_byte_disp_pic());
}

void
show_disppic_hw_rev(void) {
    int version;
    version = display_get_hw_revision();
    printf("This is hardware revision %d\n\r", version);
}

void
show_disppic_version(void) {
    int version;
...
m_display(void) {
    ...
    while (1) {
        ...
        switch (tolower(input[0])) {
            case 's':
                set_display_str(input);
                break;
            case 'u':
                return;
                break;
            case 'v':
                show_disppic_version();
                break;
            case 'w':
                show_disppic_hw_rev();
                break;
            case 'n':
                read_next_disppic_byte();
                break;
            case 'r':
                display_reset();
                break;
...
void
coil_get_mpu_dips(void) {
    int i, value;

    //we re read the dips as we are in testmode here
    lisy35_coil_read_mpu_dips();

    for (i = 0; i <= 3; i++) {
        value = lisy35_coil_get_mpu_dip(i);
        printf("MPU dip %d is %d\n\r", i + 1, value);
    }
}

void
show_coilpic_version(void) {
    int version;
...
void
coil_pulse_solenoid(char* input) {
    int coil, solenoid;
    ...
    while (1) {
        ...
        switch (tolower(input[0])) {
            case 'u':
                ...
                break;
            case 'o':
                coil_set_str(input, 1);
                break;
            case 'v':
                show_coilpic_version();
                break;
            case 'f':
                coil_set_str(input, 0);
                break;
            case 'p':
                coil_set_str(input, 2);
                break;
            case 's':
                coil_pulse_solenoid(input);
                break;
            case 't':
                coil_test();
                break;
    ...
    while (1) {
        ...
        switch (tolower(input[0])) {
            ...
            case 'm':
                ...
                break;
            case 'n':
                read_next_swpic_byte();
                break;
            case 's':
                show_closed_switches();
                break;
            case 'v':
                show_swpic_version();
                break;
