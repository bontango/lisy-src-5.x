...
int
lisy80_file_write_dipfile(int mode, char* line) {
    static FILE* fstream;
    ...
    if (mode == 0) {
        ...
        if ((fstream = fopen(dip_file_name, "w+")) == NULL)
            return -1;
        //send header line
        fprintf(fstream, "Switch;ON_or_OFF;comment (%s) mame-ROM:%s.zip\n", lisy80_game.long_name,
                lisy80_game.gamename);
        //send first line
...
//mode == 2 -> appemnd the line, close file; set /lisy to ro
int
lisy35_file_write_dipfile(int mode, char* line) {
    static FILE* fstream;
    char dip_file_name[80];
...
//mode == 2 -> appemnd the line, close file; set /lisy to ro
int
lisy1_file_write_dipfile(int mode, char* line) {
    static FILE* fstream;
    char dip_file_name[80];
...
//switch_nr is 0..3
int
lisy35_file_get_mpudips(int switch_nr, int debug, char* dip_setting_filename) {
...
//switch_nr is 0..3
int
lisy80_file_get_mpudips(int switch_nr, int debug, char* dip_setting_filename) {
...
//switch_nr is 0..2
int
lisy1_file_get_mpudips(int switch_nr, int debug, char* dip_setting_filename) {
...
int
lisy80_file_get_gamename(t_stru_lisy80_games_csv* lisy80_game) {
    ...
    if (dip_switch_val >= LISY80_FIRST_80B)
        lisy80_game->is80B = 1;
    else
        lisy80_game->is80B = 0;
    ...
    if (ls80opt.bitv.sevendigit)
        strcpy(games_csv, LISY80_GAMES_7DIGIT_CSV);
    else
        strcpy(games_csv, LISY80_GAMES_CSV);
...
int
lisy1_file_get_gamename(t_stru_lisy1_games_csv* lisy1_game) {
...
int
lisy35_file_get_gamename(t_stru_lisy35_games_csv* lisy35_game) {
...
int
lisy80_file_get_soundopts(void) {
    ...
    while ((line = fgets(buffer, sizeof(buffer), fstream)) != NULL) {
        ...
        if (sound_no < 64) {
            ...
            fprintf(stderr, "soundnumber is too big:%d in lisy80_sound_stru\n", sound_no);
    } //while
    fclose(fstream);
...
int
lisy35_file_get_soundopts(void) {
    ...
    for (i = 0; i <= 255; i++)
        lisy35_sound_stru[i].soundnumber = 0;
    while ((line = fgets(buffer, sizeof(buffer), fstream)) != NULL) {
        if (first_line) {
            first_line = 0;
            continue;
...
lisy1_file_get_coilopts(void) {
    ...
    for (i = 0; i <= 7; i++)
        lisy1_coil_min_pulse_time[i] = 150;
...
int
lisy_file_get_led_mappings(unsigned char system) {
    char buffer[1024];
    char* line;
    ...
    if (system == 1)
        sprintf(fadecandy_file_name, "%s%03d%s", LISY1_FADECANDY_PATH, lisy1_game.gamenr, LISY1_FADECANDY_GI_FILE);
    else if (system == 80)
        sprintf(fadecandy_file_name, "%s%03d%s", LISY80_FADECANDY_PATH, lisy80_game.gamenr, LISY80_FADECANDY_GI_FILE);
    else if (system == 35)
        sprintf(fadecandy_file_name, "%s%03d%s", LISY35_FADECANDY_PATH, lisy35_game.gamenr, LISY35_FADECANDY_GI_FILE);
    else
        sprintf(fadecandy_file_name, "not there");
    ...
    while ((line = fgets(buffer, sizeof(buffer), fstream)) != NULL) {
        ...
        if (led_no > 511)
            continue;                                     //skip line if LED number is out of range
    ...
    //construct the filename; using global var lisy_gamenr
    if (system == 1)
        sprintf(fadecandy_file_name, "%s%03d%s", LISY1_FADECANDY_PATH, lisy1_game.gamenr, LISY1_FADECANDY_LAMP_FILE);
    else if (system == 80)
        sprintf(fadecandy_file_name, "%s%03d%s", LISY80_FADECANDY_PATH, lisy80_game.gamenr, LISY80_FADECANDY_LAMP_FILE);
    else if (system == 35)
        sprintf(fadecandy_file_name, "%s%03d%s", LISY35_FADECANDY_PATH, lisy35_game.gamenr, LISY35_FADECANDY_LAMP_FILE);
    else
        sprintf(fadecandy_file_name, "not there");
    ...
    while ((line = fgets(buffer, sizeof(buffer), fstream)) != NULL) {
        ...
        if (lamp_no > 51)
            continue; //skip line if lamp number is out of range
...
int
lisymini_file_get_gamename(t_stru_lisymini_games_csv* lisymini_game) {
...
int
lisy_file_get_home_mappings(void) {
    char buffer[1024];
    char* line;
    char file_name[80];
    int no;
...
//fill structure stru_lisymini_games_csv
int
lisy_file_get_volume_level(void) {
    char level[20];
...
//fill structure stru_lisymini_games_csv
int
lisy_file_get_welcome_msg(char* message) {
...
//fill structure stru_lisymini_games_csv
int
lisyapc_file_get_gamename(t_stru_lisymini_games_csv* lisymini_game) {
...
int
lisy_file_get_home_ss_lamp_mappings(int variant) {
    char buffer[1024];
    char* line;
    char file_name[80];
    int no;
    ...
    if (fstream == NULL) {
        ...
        while ((line = fgets(buffer, sizeof(buffer), fstream)) != NULL) {
            ...
            if (lisy_home_ss_lamp_map[no].no_of_maps > 8)
                continue; //8 mappings in maximum
            ledline = lisy_home_ss_lamp_map[no].mapped_to_line[lisy_home_ss_lamp_map[no].no_of_maps - 1] =
                atoi(strtok(NULL, ";")); //line of led
            led = lisy_home_ss_lamp_map[no].mapped_to_led[lisy_home_ss_lamp_map[no].no_of_maps - 1] =
                atoi(strtok(NULL, ";")); //led number in this line
            //now read colorcodes
            ...
            if ((ledline <= 6) & (led <= 48)) {
                ...
                if (ls80dbg.bitv.lamps) {
                    sprintf(debugbuf, "Map Lamp %d TO line:%d led:%d %d %d %d %d\n", no, ledline, led,
                            led_rgbw_color[ledline][led].red, led_rgbw_color[ledline][led].green,
...
//fill structure
int
lisy_file_get_home_ss_coil_mappings(int variant) {
    char buffer[1024];
    char* line;
    char file_name[80];
    int no;
    ...
    if (fstream == NULL) {
        ...
        while ((line = fgets(buffer, sizeof(buffer), fstream)) != NULL) {
            ...
            if (no > 19)
                continue; //skip line if coil number is out of range
...
//fill structure
int
lisy_file_get_home_ss_special_coil_mappings(int variant) {
    char buffer[1024];
    char* line;
    char file_name[80];
    int no;
    ...
    if (fstream == NULL) {
        ...
        while ((line = fgets(buffer, sizeof(buffer), fstream)) != NULL) {
            ...
            //debug
            if (ls80dbg.bitv.lamps | ls80dbg.bitv.coils) {
                sprintf(debugbuf, "LISY HOME:  map special coil %d to number:%d (%s) pulsetime:%dms delay:%dms", no,
                        lisy_home_ss_special_coil_map[no].mapped_to_coil, lisy_home_ss_special_coil_map[no].comment,
                        lisy_home_ss_special_coil_map[no].pulsetime, lisy_home_ss_special_coil_map[no].delay);
                lisy80_debug(debugbuf);
            }
...
int
lisy200_file_get_mpudips(int switch_nr, int debug, char* dip_setting_filename) {
...
int
lisy_file_get_home_ss_special_lamp_mappings(int variant) {
    char buffer[1024];
    char* line;
    char file_name[80];
    int no;
    ...
    if (fstream == NULL) {
        ...
        while ((line = fgets(buffer, sizeof(buffer), fstream)) != NULL) {
            ...
            if (lisy_home_ss_special_lamp_map[no].no_of_maps > 8)
                continue; //8 mappings in maximum
            ledline =
                lisy_home_ss_special_lamp_map[no].mapped_to_line[lisy_home_ss_special_lamp_map[no].no_of_maps - 1] =
                    atoi(strtok(NULL, ";")); //line of led
            led = lisy_home_ss_special_lamp_map[no].mapped_to_led[lisy_home_ss_special_lamp_map[no].no_of_maps - 1] =
                atoi(strtok(NULL, ";")); //led number in this line
            //now read colorcodes
            //sanity check (no color mapping for line 7 which is coil map)
            if ((ledline <= 6) & (led <= 48)) {
...
//mode == 2 -> appemnd the line, close file; set /lisy to ro
int
lisy200_file_write_dipfile(int mode, char* line) {
    static FILE* fstream;
    char dip_file_name[80];
...
int
lisy_file_get_home_ss_GI(int variant) {
    char buffer[1024];
    char* line;
    char file_name[80];
    int no;
...
//fill structure stru_lisy35_sound_csv
int
lisy200_file_get_soundopts(void) {
...
int
lisy_file_get_home_ss_general(void) {
    char buffer[1024];
    char* line;
    char file_name[80];
    int no, param;
